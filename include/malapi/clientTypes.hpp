...
// Date A specific time. A ISO 8601 formatted time string.
typedef std::string Date;

// SortingMethodEnum The method of sorting the entries in the response lists.
typedef std::string
    SortingMethodEnum; //  One of anime_title, list_score, list_updated_at, anime_start_date

// ListStatusEnum The list status of the user.
typedef std::string
    ListStatusEnum; //  One of watching, completed, on_hold, dropped, plan_to_watch
...
struct AnimeObject {
	struct node {
		...
		std::uint16_t
		    average_episode_duration; //  The average duration (in seconds) of the episodes.
		...
		std::string
		    media_type; //  The type of this media (tv, ova, movie, special, ona, music, unknown).
		...
		std::uint16_t
		    num_favorites; //  The number of users that added this media to their favorites.
		std::uint16_t
		    num_list_users; //  The number of uses that added this media to their lists.
		std::uint16_t
		    num_scoring_users; //  (?) The number of users that voted for the scores.
